...
TEST_EPOCHS         = 1 # how often we run tests to eval our network, one epoch is one entire ppo update cycle
...
def rendering() :
    global train_epoch
    global RENDERING_EPOCHS
    global model
    global model_changed
    #if train_epoch % RENDERING_EPOCHS == 0:
    #    print("EPOCH : ", train_epoch)
    while True :
        ################# Monitoring

        show_model = model
        cur_epoch = train_epoch
        ...
        for i in range(20):  # numner of videos
            ...
            total_reward = 0
            while not done:
                show_state = torch.FloatTensor(show_state).unsqueeze(0).to(device)
                dist, _ = show_model(show_state)
                ...
                show_next_state, reward, done, _ = env.step(action)
...
if __name__ == "__main__":
    ...
    load_model = True
    if load_model :
        model = torch.load("./checkpoints/Humanoid-v2_best_+904.789_1430528.dat")
    else :
    ...
    t = threading.Thread(target=rendering)
    t.daemon = True
    t.name = "render"
    t.start()
